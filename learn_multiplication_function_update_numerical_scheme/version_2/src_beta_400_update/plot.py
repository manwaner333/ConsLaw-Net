...
def f_real_function(u):
    f = 0.5 * u * (3 - np.power(u, 2)) + (1/12) * (400) * np.power(u, 2) * ((3/4) - 2*u +(3/2)*np.power(u, 2) - (1/4)*np.power(u, 4))
    return f
def f_predict_function(u):
    # f = (-35.77093958920373)*u**3+(22.97439292755169)*u**4+(12.998084618116492)*u**2+(2.9189499211947307)*u+(-2.1083078966406066)*u**5+(0.41645624839651485)*1+(0.07582594094857291)*u**6+(-0.0012036383813980095)*u**7+(7.0509717718878516e-06)*u**8
    f = (-7.773379964300626)*u**2+(6.407142282265922)*u+(1.799468170663229)*u**3+(0.27052792274898046)*u**4+(-0.08991075806181363)*1+(0.015349513650678265)*u**5+(0.0004232001815684011)*u**6+(5.282314835677824e-06)*u**7
    return f
...
if __name__ == "__main__":
    test_data_name = 'N_200_example_6_dt_0.1_layer_10_beta_400'
    #
    # time_steps_real = 299
    # time_steps_predict = 490
    # cell_numbers = 200
    # example_index = 4
    # time = 2
