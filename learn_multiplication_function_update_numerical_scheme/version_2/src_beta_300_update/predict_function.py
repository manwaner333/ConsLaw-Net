...
class VariantCoeLinear1dPredict(torch.nn.Module):
    ...
    def f_real(self, u):
        ...
        # f = (90.6817784357376)*u**4+(-77.39591243074955)*u**3+(-51.547551433940576)*u**5+(25.267488738158164)*u**2+(15.192431521636841)*u**6+(-2.2352613741001917)*u**7+(0.8949252384294735)*u+(0.23645493043386373)*1+(0.12988412334149524)*u**8
        # f = (82.64554869125719)*u**4+(-70.98501279548253)*u**3+(-46.33261830599121)*u**5+(22.86117882073608)*u**2+(13.426869519478704)*u**6+(-1.9406607088594359)*u**7+(1.2120584601411524)*u+(0.2732339734561027)*1+(0.1107771626969545)*u**8
        f = (64.56980320533334)*u**4+(-59.0163628653965)*u**3+(-32.76874992552106)*u**5+(19.214070479074234)*u**2+(8.414033745248974)*u**6+(1.6037074321113751)*u+(-1.0640130405376145)*u**7+(0.26548926604511974)*1+(0.05271742806649773)*u**8
        return f
    ...
    def update(self):
        ...
        if max_f_prime > 0.2 and max_f_prime < 7:
...
if __name__ == "__main__":
    experiment_name = 'N_200_example_6_dt_0.1_layer_10_beta_300'
