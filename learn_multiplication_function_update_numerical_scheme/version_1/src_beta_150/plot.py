...
def f_real_function(u):
    f = 0.5 * u * (3 - np.power(u, 2)) + (1/12) * (150) * np.power(u, 2) * ((3/4) - 2*u +(3/2)*np.power(u, 2) - (1/4)*np.power(u, 4))
    return f
def f_predict_function(u):
    f = (3.1885851719198293)*u+(-2.7472568262598673)*u**2+(0.47183846479222114)*u**3+(0.289744242708718)*1+(0.08848776113200277)*u**4+(-0.006099553607475073)*u**5+(0.00012866236003297065)*u**6+(-1.0560988688166398e-06)*u**7
    return f
...
if __name__ == "__main__":
    test_data_name = 'N_200_example_1_dt_0.1_layer_10_beta_150'  # _no_stableloss

    # time_steps_real = 64   # 64, 128
    # time_steps_predict = 64  # 64, 128
    # cell_numbers = 200
    # example_index = 0
    # time = 1
