...
def plot_fixed_time_step(real_data, predict_data, time_steps_real, time_steps_predict, example_index,cell_numbers, name):
    ...
    save_dir = 'figures/' + name + '_' +'time_step_' + str(time_steps_real)
    if not os.path.isdir(save_dir):
        os.mkdir(save_dir)
    file_name = '/' + 'fixed_time_step_real_predict_data' + '_' + 'example' + '_' + str(example_index) + '_' + 'time_steps' + '_' + str(time_steps_real) + '_' + 'cell' + '_' + str(cell_numbers) + '.pdf'
...
def f_predict_function(u):
    f = (1.3759324659641288)*u+(0.47234793027481037)*1+(-0.19784297727197722)*u**3+(-0.17741007521688573)*u**2+(0.05041844680003807)*u**4+(-0.0032573211662980596)*u**5+(7.590454152793352e-05)*u**6
    return f
...
if __name__ == "__main__":
    test_data_name = 'N_200_example_1_dt_0.1_layer_10_beta_negative_100'  # _no_stableloss

    time_steps_real = 84  # 374  # 249  # 124  # 499
    time_steps_predict = 85
    cell_numbers = 200
    example_index = 0
    ...
    # 固定时间下真实和预测情况
    # plot_fixed_time_step(real_data, predict_data, time_steps_real, time_steps_predict, example_index, cell_numbers, name=test_data_name)
