...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def update(self):
        ...
        if 0.001 < max_f_prime  and max_f_prime < 20:  # 原来的（0.04，3）
...
def generate_real_data(save_file, u0_file, u_fixed_file):
    ...
    # u_0_np[:1, 400:600] = 1.0  # N = 1000
    # u_0_np[1:2, 400:600] = 0.8
    # u_0 = torch.from_numpy(u_0_np)
    # u_0 = u_0.to(device)
    batch_size = 3
    u_0_np = np.zeros((batch_size, N), dtype=float)
    u_0_np[0:1, 400:600] = 1.0  # N = 1000
    u_0_np[1:2, 400:600] = 0.8
    u_0_np[2:3, 400:600] = 0.5
    u_0 = torch.from_numpy(u_0_np)
    u_0 = u_0.to(device)
    # 引入 u_fixed, 用来计算max_f_prime
    du = 1.2/52
...
if __name__ == "__main__":
    experiment_name = 'N_1000_example_3_dt_0.1_layer_10_beta_0.5_no_alpha'
