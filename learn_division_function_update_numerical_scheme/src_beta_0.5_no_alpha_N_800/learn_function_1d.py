...
options = {
    ...
    '--precision': 'double',
    '--taskdescriptor': 'N_800_example_2_dt_0.1_layer_10_beta_0.5_no_alpha',
    '--batch_size': 2,  # 250,  # 150,
    '--maxiter': 200,   # 156,
    '--X': 10,
    '--T': 2.0,
    '--dx': 0.0125,  # 0.0125,   # 0.025,  # 0.125,
    '--N': 800,    # 800,  # 400,    # cell number
    '--M': 0.1100,
    '--dt': 0.169492,
    '--time_steps': 236,
    '--layer': 10,  # time steps
...
def initexpr(model):
    ...
    for poly in rhi:
        for p in poly.parameters():
            p.data = torch.randn(*p.shape, dtype=p.dtype, device=p.device) * 1e-1 * 6    # * 1e-1 * 6
                     # * 1e-1 * 6
    return None
